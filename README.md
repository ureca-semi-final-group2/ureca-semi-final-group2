# 🐙 말 많은 무너팀 (유레카 3기 대면 백엔드) 

<div align="center">
  <img src="https://github.com/user-attachments/assets/9a54a772-af78-4451-bc60-0528c480b4d6" width="470" alt="말많은무너팀 로고"
  />


  <h3>"말은 많지만 결과는 확실한, 유레카 대면 종합 프로젝트"</h3>

  <p>대용량 통신 요금 명세서 및 알림 발송 시스템</p>

  [![Build Status](https://img.shields.io/badge/build-passing-brightgreen)](#)
  [![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)
</div>

<br/>

## 📌 목차
1. [프로젝트 소개](#-프로젝트-소개)
2. [주요 기능](#-주요-기능)
3. [아키텍처](#-아키텍처)
4. 기술적 특이점 및 도전 과제
5. 데이터베이스 설계 (Single Source of Truth)
6. [발송 프로세스 & Kafka 로직](#-발송-프로세스--kafka-로직)
7. [Billing 테이블 설계](#-billing-테이블-설계)
8. [기술 스택](#-기술-스택)
9. [시작 가이드](#-시작-가이드)
10. [팀원 소개](#-팀원-소개)




<br>

## 📝 1. 프로젝트 소개
* **개발 기간**: 2026.01.06 ~ 2026.01.27
* **서비스 명**: [대용량 통신 요금 명세서 및 알림 발송 시스템]
* **핵심 목표**: 통신사의 핵심 업무인 대규모 정산과 알림 발송을 안정적으로 처리하기 위한 시스템입니다. 매월 수백만 건에 달하는 요금 청구 데이터를 배치 기반으로 정확하게 정산하고, 정산 결과를 기반으로 요금 청구서를 고객에게 이메일과 SMS를 통해 발송합니다.
  
* **배포 주소**: [🚀 서비스 바로가기 링크]()


<br>

## ✨ 2. 주요 기능
* **✅ 1. 요금 정산 배치 시스템**: 정해진 날짜에 배치 기반 요금 정산 수행

* **✅ 2. 모니터링 툴 시스템**: 대용량 배치 작업 현황 및 시스템 리소스 모니터링
  - **메트릭 수집**: Prometheus를 통해 시스템 자원 및 배치 수행 지표를 수집한다.
  - **메트릭 시각화**: Grafana를 통해 수집된 메트릭을 대시보드로 시각화하고 시스템 상태를 모니터링한다.
    
* **✅ 3. 이벤트 기반 알림 발송 시스템**: 요금 정산 완료시 이벤트를 기반으로 Kafka를 활용해 이메일/SMS 발송
* 분산 발송 전략: 시스템 부하 분산을 위해 사용자별 지정 발송일(15일, 21일)에 맞춰 배치를 구동하고 Kafka 이벤트를 발행합니다. 
  - **비동기 발송**: Kafka Consumer를 통해 이메일 및 SMS를 비동기 발송(Mocking) 처리한다.
  - **금지 시간대 제어**: Redis에 설정된 금지 시간대에는 발송을 일시 정지하고 대기 큐로 돌린다.
  - **재시도 전략**: 이메일 발송 실패(1% 확률)시 SMS로 청구 내역을 재발송한다.

* **✅ 4. 프론트 관리자 관제**
  - **실시간 대시보드**: 정산 진행률, Kafka Lag, 발송 성공/실패율을 시각화하여 보여준다.
  - **발송 관리자 설정**: 유저 검색을 통한 **[강제 발송]** (금칙 시간 무시 옵션), DLQ 로그 기반의 **[수동 SMS 전환 발송]** 기능을 통해 신속히 대응합니다.



## 🏗 3. 아키텍처
<img width="8192" height="3178" alt="Billing Kafka Data Pipeline-2026-01-26-133835" src="https://github.com/user-attachments/assets/72f95eba-12fa-4500-adc1-378579386717" />


### 1단계 월간 정산


### 2단계 발송 대상 확정 및 Kafka 발행


### 3단계 실시간 DND 필터링


### 4단계 비동기 발송 및 트랜잭션 분리


### 5단계 재발송 및 복구


## 🚀 4. 기술적 특이점 및 도전 과제



## 📤 4. 발송 프로세스 & Kafka 로직


## 🧾 5. Billing 테이블 설계

### Primary Key (복합키)
| 컬럼 | 타입 | 설명 |
|---|---|---|
| id | BIGINT | 사용자 청구 ID |
| billing_date | TIMESTAMP | 청구 기준 월/일 (파티셔닝 키) |


### 주요 컬럼
| 컬럼 | 설명 |
|---|---|
| billingFee | 청구 금액 |
| status | 결제 상태 (PayStatus) |
| sendStatus | 발송 상태 (SendStatus) |
| paidDate | 결제 완료 시점 |
| billingDetails | 청구 상세(JSONB) |


### SendStatus 정의
```java
public enum SendStatus {
    CREATED, // 정산 배치 완료
    SEND_PENDING, // 발송 대상 확정
    IN_QUIET_HOUR, // 금칙 시간 대기
    COMPLETED, // 발송 성공
    FAILED // 발송 실패
}
```






## 발송 프로세스와 금칙 시간 처리 
### 1. 서비스 설계: 왜 15일과 21일인가? (Business Logic)
부하 분산 전략: 500만 건의 데이터를 정산 직후(1일) 한꺼번에 발송하지 않고, 15일과 21일로 나누어 시스템 부하를 물리적으로 분산했습니다.

고객 지향적 옵션: 한국 기업의 주요 급여일(10일, 17일, 25일)과 신용카드 결제 대금일을 고려하여, 고객이 자금 계획을 세우기 가장 합리적인 시점인 '중순(15일)'과 '하순(21일)'을 옵션으로 제공했습니다.

### 2. 사용자별 금칙 시간(DND) 처리
문제 상황: 100만 명 이상의 유저가 각기 다른 금칙 시간 정책(예: 22시~08시)을 가지고 있음.

핵심 고민: 금칙 시간에 걸린 수백만 건의 메시지를 '성능 저하 없이' 어디에 보관하고, 제시간에 어떻게 깨울 것인가?

Redis ZSET vs Message Queue (RabbitMQ)



### 3. kafka 상세 로직
발송 트리거: 15일/21일 00시에 스케줄러가 대상을 추출해 Kafka로 전달.

실시간 필터링: 컨슈머가 Redis 정책을 조회해 '금칙 시간' 여부를 판단.

지연 큐 적재: 금칙 시간 유저는 **해제 시점(Timestamp)**을 Score로 하여 Redis ZSET에 저장.

자동 재발행: 1시간 주기 스케줄러가 ZRANGEBYSCORE로 해제된 유저만 추출하여 다시 Kafka 발송 토픽으로 재발행(Re-injection).


## 🛠 기술 스택

### Environment
<p>
  <img src="https://img.shields.io/badge/git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white">
  <img src="https://img.shields.io/badge/github-%23121011.svg?style=for-the-badge&logo=github&logoColor=white">
  <a href= "https://shadow-lychee-a03.notion.site/2e0ab48eeb4b8001bb42f7c35e987cd8?source=copy_link">
    <img src="https://img.shields.io/badge/Notion-%23000000.svg?style=for-the-badge&logo=notion&logoColor=white">
  </a>
  <a href="https://jack36140.atlassian.net/jira/software/projects/MOONU/summary" target="_blank">
  <img src="https://img.shields.io/badge/jira-0052CC?style=for-the-badge&logo=jira&logoColor=white">
</a>
</p>

### Development
<p>
  <img src="https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white">
  <img src="https://img.shields.io/badge/spring%20boot-%236DB33F.svg?style=for-the-badge&logo=springboot&logoColor=white">
  <img src="https://img.shields.io/badge/react-%2320232a.svg?style=for-the-badge&logo=react&logoColor=%2361DAFB">
  <img src="https://img.shields.io/badge/postgresql-%234479A1.svg?style=for-the-badge&logo=postgresql&logoColor=white">
</p>
노션 링크에 저희 팀의 문서화가 자세히 되어있습니다.

---

## 🚀 시작 가이드

### 요구 사항 (Prerequisites)
* Java 17+
* Spring Boot 3.4.1
* PostgreSQL 18
* Kafka 3.5 (Confluent 7.5.0)

---

## 👥 팀원 소개

| 🐙 무너 1호 | 🐙 무너 2호 | 🐙 무너 3호 | 🐙 무너 4호 | 🐙 무너 5호 | 🐙 무너 6호 |
| :---: | :---: | :---: | :---: | :---: | :---: |
| <img src="https://github.com/hoonseok0710.png" width="100"/> | <img src="https://github.com/ashmin-yoon.png" width="100"/> | <img src="https://github.com/yubin012.png" width="100"/> | <img src="https://github.com/LeeGyeongYoon-BE.png" width="100"/> | <img src="https://github.com/dnwldla.png" width="100"/> | <img src="https://github.com/rettooo.png" width="100"/> |
| **최훈석 (팀장)** | **윤재민** | **박유빈** | **이경윤** | **임지우** | **최하영** |
| [@hoonseok0710](https://github.com/hoonseok0710) | [@ashmin-yoon](https://github.com/ashmin-yoon) | [@yubin012](https://github.com/yubin012) | [@LeeGyeongYoon-BE](https://github.com/LeeGyeongYoon-BE) | [@dnwldla](https://github.com/dnwldla) | [@rettooo](https://github.com/rettooo) |
| Backend / Infra | Backend / Infra | Backend / Kafka | Backend / DB | Backend / DB | Backend / Kafka |


---

### 🐙 역할
* **최훈석**: "모니터링 툴 작업, 이벤트 알림"
* **윤재민**: "배치 작업, 약정 및 생일 이벤트 할인"
* **박유빈**: "발송 배치 작업 후 Kafka 처리"
* **이경윤**: "부가 서비스 등급제 할인, 장기 고객 할인"
* **임지우**: "정산작업처리 "
* **최하영**: "대용량 데이터 카프카 처리"

